...
final class SignUtils {
    ...
    static boolean isSignatureValid(BasicOCSPResp validator, Certificate certStoreX509, String provider) throws OperatorCreationException, OCSPException {
        if (provider == null) provider = "BCFIPS";
    ...
    static boolean checkIfIssuersMatch(CertificateID certID, X509Certificate issuerCert) throws CertificateEncodingException, IOException, OCSPException {
        try {
            return certID.matchesIssuer(new X509CertificateHolder(issuerCert.getEncoded()), new JcaDigestCalculatorProviderBuilder().build());
    ...
    static Collection<Certificate> readAllCerts(byte[] contentsKey) throws CertificateException  {
        ...
        return (Collection<Certificate>) factory.generateCertificates(new ByteArrayInputStream(contentsKey));
    }
    ...
    static X500Principal getIssuerX509Name(ASN1Sequence issuerAndSerialNumber) throws IOException {
